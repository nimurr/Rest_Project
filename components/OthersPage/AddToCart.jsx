...
function AddToCart() {
    ...
    const handleAddToCartDelete = (id) => {
        ...
        const filter = deleteDataById.filter(d => d.id != id)
...
const getquantity = (id) =>{
    const productinfo = localdataInfo.find(item=>item.id==id)
    return productinfo.quantity;
    // return productinfo.quantity 
}

   
    const totalPrice = addTocart.reduce((d, i) => (d + (parseFloat(i.price * getquantity(i.id)))), 0)

    // const [totalPricein , setTotalPrice] = useState([])


    const handleDecressAddToCart = (id) => {
        const cartItem = localStorage.getItem('addToCart')
        const parsCartItem = JSON.parse(cartItem);
        const updatedData = parsCartItem.map(item => {
            if (item.id == id && item.quantity > 1) {
                // Update the quantity for the target id
                return { ...item, quantity: item.quantity - 1 }; // or set it to a specific value
            }
            return item;
        });
        ...
        localStorage.setItem('addToCart', updatedDataString)
        setProductQuantity(productQuantity - 1)


    }
    const handleIncressAddToCart = (id) => {
        const cartItem = localStorage.getItem('addToCart')
        const parsCartItem = JSON.parse(cartItem);
        const updatedData = parsCartItem.map(item => {
            if (item.id == id) {
                        ...
                        </div>
                        {
                            addTocart?.map(products =>
                                <div data-aos="zoom-in" className="sm:flex rounded-md justify-between items-center gap-4 mb-5 bg-gray-100 p-4" key={products.id}>
                                    ...
                                    <div className="flex ">
                                        <button onClick={() => handleDecressAddToCart(products.id)} className="text-2xl font-semibold bg-gray-200 px-2">-</button>
                                        <span id="productQuantity" className="bg-white px-2 flex items-center" >
                                            {
                                                localdataInfo.find(item => item.id == products.id).quantity


                                            }
                                        </span>
                                        <button onClick={() => handleIncressAddToCart(products.id)} className="text-2xl font-semibold bg-gray-200 px-2">+</button>
                                    </div>
                                    <div>
